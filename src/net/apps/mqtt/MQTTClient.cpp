...
void MQTTClient_init(IpAddr server, int port, void (*callback)(char*, char*))
{
    this->port = port;
...
int MQTTClient_send_data(const char* msg, int size)
{
...
int MQTTClient_open_session(char* id)
{
    /*variable header*/
...
int MQTTClient_connect(char* id)
{
    clientId = id;
...
int MQTTClient_publish(char* pub_topic, char* msg) 
{
...
void MQTTClient_read_open_session()
{
    char buffer[32];
    int len = 0, readLen;
